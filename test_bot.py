...
# Mock environment setups prior to importing stream_bot
...
class TestDiscordStreamBotFullCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        # Create a clean mock bot instance for every independent runtime pass
    ...
    def test_save_stream_state_exception(self):
        """Ensures exceptions inside save_stream_state are trapped safely without crashes."""
    ...
    async def test_subcommand_start_not_in_voice(self):
        ...
        group = self.bot.tree.get_commands()
        start_cmd = group.get_command('start')
        
        interaction, _ = self._create_mock_interaction(in_voice=False)
        await start_cmd._callback(interaction)
    ...
    async def test_subcommand_volume_not_streaming(self):
        ...
        await self.bot.setup_hook()
        group = self.bot.tree.get_commands()
        vol_cmd = group.get_command('volume')
        
        interaction, _ = self._create_mock_interaction(streaming=False)
        await vol_cmd._callback(interaction, percentage=50)
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_volume_invalid_transformer(self):
        ...
        await vol_cmd._callback(interaction, percentage=75)
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_volume_success(self):
        ...
        await vol_cmd._callback(interaction, percentage=80)
    ...
    async def test_subcommand_sleep_disconnected(self):
        ...
        await self.bot.setup_hook()
        group = self.bot.tree.get_commands()
        sleep_cmd = group.get_command('sleep')
        
        interaction, _ = self._create_mock_interaction(streaming=False)
        await sleep_cmd._callback(interaction, duration="15m")
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_sleep_invalid_unit(self):
        ...
        await sleep_cmd._callback(interaction, duration="10x")
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_sleep_invalid_format(self):
        ...
        interaction, _ = self._create_mock_interaction(streaming=True)
        await sleep_cmd._callback(interaction, duration="garbage_string")
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_sleep_relative_execution(self):
        ...
        self.bot.sleep_tasks = mock_old_task
        
        with patch('asyncio.sleep', AsyncMock()) as mock_async_sleep, \
             patch('stream_bot.clear_stream_state') as mock_clear:
            
            await sleep_cmd._callback(interaction, duration="2s")
            mock_old_task.cancel.assert_called_once()
            
            # Extract and manually complete background worker execution task thread lines
            worker_task = self.bot.sleep_tasks
            await worker_task
    ...
    async def test_subcommand_sleep_absolute_rollover(self):
        ...
        # Lock system time to noon, and challenge parser to map a targeted past marker like '11:00 AM'
        ...
        with patch('stream_bot.datetime') as mock_dt, patch('asyncio.create_task'):
            ...
            await sleep_cmd._callback(interaction, duration="11:00am")
            # Proves delta generation added exactly 23 hours to bridge the roll boundary
    ...
    async def test_subcommand_wake_not_in_voice(self):
        ...
        await self.bot.setup_hook()
        group = self.bot.tree.get_commands()
        wake_cmd = group.get_command('wake')
        
        interaction, _ = self._create_mock_interaction(in_voice=False)
        await wake_cmd._callback(interaction, duration="1h")
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_wake_invalid_unit(self):
        ...
        await wake_cmd._callback(interaction, duration="50z")
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_wake_invalid_format(self):
        ...
        interaction, _ = self._create_mock_interaction(in_voice=True)
        await wake_cmd._callback(interaction, duration="bad_time_string")
        interaction.response.send_message.assert_called_once_with(
    ...
    async def test_subcommand_wake_execution_with_active_disconnect(self):
        ...
        self.bot.wake_tasks = mock_old_wake
        ...
        with patch('asyncio.sleep', AsyncMock()), \
            ...
            await wake_cmd._callback(interaction, duration="1s")
            mock_old_wake.cancel.assert_called_once()
            
            worker = self.bot.wake_tasks
            await worker
