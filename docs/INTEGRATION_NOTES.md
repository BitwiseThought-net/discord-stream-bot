> **STATUS: APPLIED.** These changes are now merged directly into `bot.py`
> (architecture/KVM/timeout helpers, `_spawn_android_emulator_stack`,
> compose teardown, and the `android-ui` command). This file and
> `android_emulator_updates.py` are kept as a historical record of what
> changed and why -- including a real regression that was caught and fixed
> along the way (see below) -- not as something still to be applied.
> `bot.py`'s actual implementation differs slightly in one respect: it uses
> `docker compose up -d --wait --wait-timeout <N>` instead of a hand-built
> `_run_docker_cmd` polling helper, since Compose's own `--wait` flag does
> the same job (block until healthy, via Popen, no `subprocess.run`) more
> simply and is what actually shipped.
